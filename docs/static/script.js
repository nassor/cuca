...
(() => {
  ...
  // only looks at `class="language-*"`. Without this bridge no fence on the
  // site is highlighted, because every code block here comes from markdown.
  //
  ...
  document.addEventListener('DOMContentLoaded', () => {
    ...
    if (themeToggle) {
      ...
      themeToggle.addEventListener('click', () => {
        ...
        try { localStorage.setItem('cuca:theme', next); } catch (e) {}
        label();
      });
    ...
    const GROUPS_KEY = 'cuca:nav-groups';
    let savedGroups = {};
    ...
    if (tocHost && headings.length > 1) {
      ...
      //
      // The insert is anchored on the heading's own parent, not on `content`:
      // markdown bodies render inside `<article class="markdown">`, so the
      // first `h2` is a grandchild of `<main>` and `content.insertBefore`
      // would throw. A throw here would abort the rest of this listener,
      // taking the code frames, the copy buttons and search down with it.
      ...
      const place = () => {
        ...
        if (wide.matches) {
          ...
          tocHost.hidden = true;
          headings[0].parentNode.insertBefore(details, headings[0]);
        }
      };
      place();
