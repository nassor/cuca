...
**Tiers.** Core (`types`, `request`, `error`, `sse`, `session`, `plugin`, `client`, `provider`, `export`, `canonical`) sits below every plugin. Foundational plugins depend on core only: `memory`, `session-log`, `mcp`, `sandbox`, `guardrails`, `subagent`, `hitl`, `web-search`, `skills`, `telemetry`, `prompt-cache`. Derived plugins depend on the foundational plugins they declare: `entity-extraction → memory` (hard, `plugin-entity-extraction = ["plugin-memory"]`) and `speculative → session-log` (documented-optional: `ModelOrchestrator::with_session_store` records `SessionEvent::ModelSwap`).
