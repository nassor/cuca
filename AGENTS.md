...
Current state: **implemented**. The unified client (`CucaClient`/`CucaClientBuilder`), the zero-allocation SSE parser, seven provider adapters, seventeen feature-gated plugins, and the speculative fast/slow orchestrator are all implemented.
...
cargo test --features "provider-llamacpp plugin-mcp plugin-sandbox plugin-memory plugin-entity-extraction plugin-guardrails plugin-subagent plugin-hitl plugin-web-search plugin-skills plugin-telemetry plugin-speculative plugin-session-log plugin-replay plugin-prompt-cache plugin-cost plugin-rate-limit plugin-redaction"   # live llama.cpp integration suite (see Testing & QA)
...
- **Explicit-call capabilities:** a capability driven by direct method calls rather than pipeline hooks MUST NOT implement `CucaPlugin` (`PromptCache`, `EntityExtractionPlugin`, `SessionReplay`), so registering it is a compile error instead of an inert no-op; its `//!` header names the real entry points and any mandatory hand-off.
...
| `Cargo.toml` | Package manifest for `cuca` (v0.1.0, edition 2024, rust-version 1.98): crates.io publish metadata (description, repository, homepage, docs.rs all-features build), the 24-feature matrix (7 providers + 17 plugins), no default provider, the two hard cross-plugin feature edges (`plugin-entity-extraction = ["plugin-memory"]`, `plugin-replay = ["plugin-session-log"]`), minimal core dependencies, feature-owned transport/runtime dependencies, and dev-deps. |
