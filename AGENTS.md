...
Current state: **implemented**. The unified client (`CucaClient`/`CucaClientBuilder`), the zero-allocation SSE parser, seven provider adapters, twelve feature-gated plugins, and five explicit-call services, including the speculative fast/slow orchestrator, are all implemented.
...
cargo test --features "provider-llamacpp plugin-mcp plugin-sandbox plugin-memory service-entity-extraction plugin-guardrails plugin-subagent plugin-hitl plugin-web-search plugin-skills plugin-telemetry service-speculative plugin-session-log service-replay service-prompt-cache plugin-cost service-rate-limit plugin-redaction"   # live llama.cpp integration suite (see Testing & QA)
