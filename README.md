...
- **Everything is a plugin.** MCP connectors, WASM sandboxing, memory compression and in-memory graph memory, output guardrails, subagent delegation, human approval, web search, skills, telemetry, session logging, local response caching, speculative fast/slow routing, and schema-guided entity extraction are compile-time feature flags.
...
- The repository's `examples/` folder demonstrates the same pattern in three flavors: `cargo run --example llamacpp_gemma --features provider-llamacpp` (plain reply), `stream_all_blocks` (every block type), and `custom_plugin` (a counting plugin). The examples read `CUCA_BASE_URL` and `CUCA_MODEL` from the environment (defaults target a local llama.cpp server), e.g. `CUCA_BASE_URL=http://127.0.0.1:8000/v1 CUCA_MODEL=<server-model-id> cargo run --example llamacpp_gemma --features provider-llamacpp` to point at a vLLM server instead.
